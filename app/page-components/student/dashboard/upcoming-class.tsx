...
const textColorProps: Record<NonNullable<UpcomingClassProps["variant"]>, string> = {
...
const borderColorProps: Record<NonNullable<UpcomingClassProps["variant"]>, string> = {
...
const bgColorProps: Record<NonNullable<UpcomingClassProps["variant"]>, string> = {
  green: "bg-green-500",
  blue: "bg-blue-500",
  red: "bg-red-500",
...
const ringColorProps: Record<NonNullable<UpcomingClassProps["variant"]>, string> = {
...
export const UpcomingClass = (props: UpcomingClassProps) => {
  const formattedMinutes = moment.duration(props.minutes, "minutes").humanize(true);
  const formattedStudents = Intl.NumberFormat('en', { notation: 'compact' }).format(props.students);
  return (
    <div className="grid grid-cols-3 bg-[#f9f8fd] rounded-xl hover:shadow-md cursor-pointer transition-all p-4 dark:border-gray-700 dark:bg-gray-800 dark:shadow-slate-700/[.7] md:p-5">
      <CircleProgress variant={props.variant} percent={props.percent} radius={30} image={`images/subjects/${props.category}.png`}/>
      ...
      <span className={classNames("inline-block font-semibold mt-4 align-bottom text-xl text-center", textColorProps[props.variant!])}>{props.percent}%</span>
      <div className="flex justify-between col-span-2 mt-4">
        <button type="button"
                className={classNames("py-2 px-8 font-light inline-flex justify-center items-center gap-2 rounded-md border hover:text-white focus:outline-none focus:ring-2 focus:ring-offset-2 transition-all text-sm dark:focus:ring-offset-gray-800", [
        ...
        <button type="button"
                className={classNames("py-2 px-8 font-light inline-flex justify-center items-center gap-2 rounded-md border text-white hover:text-white focus:outline-none focus:ring-2 focus:ring-offset-2 transition-all text-sm dark:focus:ring-offset-gray-800", [
                  borderColorProps[props.variant!],
